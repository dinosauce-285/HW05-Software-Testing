...
Danh mục sản phẩm được sinh theo tổ hợp **7 thương hiệu x 3 thế hệ x 7 biến thể**, nhờ vậy biết
...
! **Không được dùng `node sut/backend/database.js` để reset.** File đó gọi `initDatabase()` ngay
...
|---|---|---|
| 3 test plan đúng quy ước `{MSSV}_{Scenario}_{YYYYMMDD}` | [x] | `plans/` |
| 3 file CSV riêng cho 3 nhóm endpoint | [x] | `data/` |
| 3 listener khác loại (Summary / Aggregate / View Results Tree) | [x] | trong từng `.jmx` |
| Log `.jtl` thô, nguyên vẹn - **8 lượt chạy, 1 386 000+ request** | [x] | `results/raw/` + `MANIFEST.md` (checksum SHA-256) |
| Thư mục HTML dashboard cho mỗi lượt | [x] | `results/html/` |
| CSV lấy mẫu tài nguyên cho mỗi lượt | [x] | `evidence/monitor/` |
| 14 ảnh bằng chứng lỗi | [x] | `evidence/bugs/` |
| 13 GitHub Issue | [x] | repo bài làm |
| Screenshot JMeter + `htop` **cùng một khung hình** | [x] 3 ảnh | `evidence/monitor/*-jmeter-htop.png` |
| Hardware report (2 ảnh + bảng spec) | [x] | `evidence/hardware/` |
| Flow chart mô hình CPT | [x] | `evidence/diagrams/` |
...
| Bắt buộc xác nhận bằng lần chạy thứ hai | Đưa xác suất báo động giả từ *p* xuống *p^2* |
