...
|---|---|---|
| 1 | Cho AI phân tích `.jtl` và đề xuất ngưỡng *(mục 6:102)* | ✅ `submission/appendix/AI-Analysis-Raw.md` — 7 715 từ, giữ nguyên văn |
| 2 | Săn chỗ đọc sai, kèm giá trị đúng từ `.jtl` thô *(mục 6:103)* | ✅ bảng A và B bên dưới |
| 3 | Phân loại 10 đề xuất tối ưu feasible / hallucinated *(mục 6:104)* | ✅ |
| 4 | Tổng kết tỉ lệ | ✅ |
...
|---|---|---|---|
| 1 | 0 | 2 lần (0→2→4) | 10 tài khoản × 2 = **20** ✓ |
| 2 | 4 (không reset) | 1 lần (4→6) | 10 × 1 = **10** ✓ |
| 3 | 6 | 1 lần | 10 × 1 = **10** ✓ |
...
| **Soak** | 627 943 | **có** | **8 ms** | 12 ms | 1,5× |
| **Stress** | 770 065 | **có** | **237 ms** | 1 671 ms | **7,1×** |
...
|---|---|---|---|
| **A1** | Prepared statement cho `?search=` | ✅ **Feasible** | Kiểm chứng `server.js:144` — đúng là nối chuỗi. Sửa một dòng, xoá 100% lỗi 500 (22 886 lỗi trên 2 lượt). Đồng thời bịt SQL injection |
| **A2** | Tăng backlog TCP + keep-alive | ✅ **Feasible** | `server.js:570` gọi `app.listen(PORT, cb)` — **không truyền backlog**, nên dùng mặc định 511 của Node. Đo được `Connect` p95 nhảy 1 → 1 020 ms tại 1 800 luồng, đúng chỗ AI chỉ |
| **A3** | Giới hạn vòng đời giỏ hàng trong RAM | ✅ **Feasible** | `userCarts = {}` (`server.js:14`) chỉ `push` (dòng 293), không có đường xoá. Đo được 19,0 MB không nhả lại |
| **B1** | Chạy Node ở chế độ cluster | ✅ **Feasible** | Đo được `node` chiếm 102% CPU trung bình trong khi `loadavg` toàn máy 1,53–1,93 trên 16 luồng. Cảnh báo kèm theo của AI — **A3 phải làm trước B1**, vì giỏ hàng trong RAM sẽ vỡ khi có nhiều worker — là chính xác và tinh tế |
| **B2** | Cache `GET /api/products` + ETag | ✅ **Feasible** *(ước lượng dựa trên suy luận sai)* | Bản thân giải pháp hợp lý. Nhưng cách ước lượng thì hỏng: AI lấy chênh lệch 22% giữa `/api/products` (3,37 ms) và `?search=` (2,76 ms) rồi quy ra "chi phí tuần tự hoá chiếm 20-25%". Hai endpoint khác nhau **cả ở truy vấn lẫn ở kích thước kết quả** — không tách được biến |
| **B3** | Bật SQLite WAL | ✅ **Feasible** | Một lệnh `PRAGMA`. Lợi ích hiện tại khiêm tốn vì đường ghi chưa bão hoà — chính AI cũng nói vậy, không thổi phồng. Lập luận "điều kiện bắt buộc để B1 phát huy tác dụng" là hợp lý |
| **B4** | Kết nối DB dùng chung ~~+ chỉ mục~~ | ❌ **Hallucinated** *(nửa đầu)* | *"Mở một kết nối SQLite bền vững cho mỗi worker **thay vì mở/đóng theo request**"* — SUT **chưa bao giờ** mở kết nối theo request. `database.js:5` mở **một** kết nối lúc nạp module rồi `module.exports = db` (dòng 119); `server.js` có **0** lần gọi `new sqlite3.Database`. AI tối ưu một vấn đề không tồn tại. Nửa sau (chỉ mục) thì trung thực — chính AI ghi "lợi ích gần như bằng không" ở quy mô 200 dòng |
| **C1** | Băm mật khẩu bằng bcrypt | ✅ **Feasible** | Kiểm chứng `server.js:46` so sánh chuỗi trần. AI còn chủ động cảnh báo rằng ai đọc 198,8 ms rồi quy cho chi phí băm là sai — chính là cái bẫy đề bài giăng sẵn, và nó tránh được |
| **C2** | Chuyển sang PostgreSQL | ✅ **Feasible** *(hoãn có điều kiện)* | Không đề xuất làm ngay, nêu rõ ngưỡng kích hoạt (>2 000 ghi/giây hoặc cần nhiều máy). Đúng với dữ liệu: checkout 2,9 ms, SQLite chưa phải nút thắt |
| **C3** | SQLite FTS5 khi danh mục lớn | ✅ **Feasible** *(hoãn có điều kiện)* | `LIKE '%từ khoá%'` có ký tự đại diện ở đầu thì không chỉ mục nào dùng được — đúng, kiểm chứng `server.js:144`. Vô hại ở 147 sản phẩm |
